        ...
        try {
            ...
            if (update.ok !== 0) {
                const updatedCourse = await Course.updateOne(
                    { _id: courseID },
                    { userJoined: course.userJoined + 1 },
                );
    ...
    async PostExam(req, res, next) {
        ...
        try {
            ...
            if (validateReq.error !== null) {
                ...
                if (exam) {
                    const courses = user.courses.find(
                        (e) => e.courseID == courseID,
                    );
                    if (!courses.isGraduate) {
                        ...
                        userAnswer.forEach((e, idx) => {
                            if (
                                e.answer ===
                                exam.questions[idx].correctAnswerID
                            ) {
                                correct++;
                            }
                        });
                        ...
                        res.status(200).json({
                            message:
                                score >= 80
                                    ? 'Congrats you graduated from this course'
                                    : 'So close, please try this test again',
                            score: score,
                        });
                    } else {
                        throw new NotAcceptable(
                            'You already graduated from this course',
                        );
                    }
                } else {
